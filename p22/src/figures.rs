...
impl Shape { 
    ...
    pub fn gen_string(&self) -> String {
        match self {
            ...
            format!("x: {} y: {} radius: {} area: {} perimeter: {}",
                    p.x, p.y, r, self.area(), self.perimeter())
            }
            Shape::Triangle { p1, p2, p3 } => {
                format!("x1: {} y1: {} x2: {} y2: {} x3: {} y3: {} area: {} perimeter: {}",
                p1.x, p1.y,
                p2.x, p2.y,
                p3.x, p3.y,
            ...
            Shape::Rectangle { p1, p2 } => {
                format!("x1: {} y1: {} x2: {} y2: {}  area: {} perimeter: {}",
...
mod tests {
    ...
    fn test_figures_names() {
        assert_eq!(
            (Shape::Point { p: Point {x: (1), y: (2)}}).name().as_bytes(),
            b"Point"
        );
        assert_eq!(
            (Shape::Circle {p: (Point {x: (10), y: (20)}), r: (10)}).name().as_bytes(),
            b"Circle"
        );
        assert_eq!(
            (Shape::Triangle {
    ...
    fn test_figures_areas() {
        assert_eq!(
            (Shape::Point { p: Point {x: (1), y: (2)}}).area(),
            0
        );
        assert_eq!(
            (Shape::Circle {p: (Point {x: (10), y: (20)}), r: (10)}).area(),
            314
        );
        assert_eq!(
            (Shape::Triangle {
    ...
    fn test_figures_perimeters() {
        assert_eq!(
            (Shape::Point { p: Point {x: (1), y: (2)}}).perimeter(),
            0
        );
        assert_eq!(
            (Shape::Circle {p: (Point {x: (10), y: (20)}), r: (10)}).perimeter(),
            62
        );
        assert_eq!(
            (Shape::Triangle {
    ...
    fn test_figures_circle() {
        ...
        assert_eq!(c.gen_string().as_bytes(), b"x: 10 y: 20 radius: 10 area: 314 perimeter: 62");
    }
    ...
    fn test_figures_triangle() {
        ...
        assert_eq!(t.gen_string().as_bytes(), b"x1: 5 y1: 5 x2: 10 y2: 10 x3: 10 y3: 5 area: 8 perimeter: 17");
    }
    ...
    fn test_figures_rectangle() {
        ...
        assert_eq!(rc.gen_string().as_bytes(), b"x1: 15 y1: 15 x2: 25 y2: 25  area: 100 perimeter: 40");
    }
}